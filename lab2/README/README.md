...
У даному завданні аналізується можливість використання стеку замість лічильника команд (IP) для керування послідовністю виконання коду при викликах функцій.
...
# Завдання: Використання сегмента BSS для збереження великих масивів
...
BSS (Block Started by Symbol) - це сегмент програми, який містить неініціалізовані глобальні та статичні змінні. Цей сегмент має деякі особливості:

1. Змінні в BSS сегменті ініціалізуються нулями автоматично при запуску програми
2. BSS сегмент не займає місце в виконуваному файлі (на відміну від сегмента даних)
3. BSS дозволяє виділяти значно більші обсяги пам'яті, ніж стек

У даному завданні ми створюємо великий масив (розміром приблизно 4 МБ) у сегменті BSS і демонструємо доступ до нього. Масив автоматично ініціалізується нулями, а ми змінюємо значення першого елемента і виводимо його.
...
## Компіляція
```
root@host:/home/guest/aspz_labs/lab2 # cc -Wall -o variant variant.c
root@host:/home/guest/aspz_labs/lab2 # ./variant
BSS array first element: 1
